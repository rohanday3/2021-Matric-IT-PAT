...
procedure TForm2.password_strength(Password: string);
var
  Length, number, upp, low, special: Boolean;
  s: string;
const
  numbers = [0 .. 9];
  uppercase = ['A' .. 'Z'];
...
begin
  ...
  begin
    if s in numbers then
      number := True;
    if s in uppercase then
      upp := True;
    if s in Lowercase then
      low := True;
...
const
  ...
  special_char = ['~', '`', '!', '@', '#', '$', '%', '^', '&', '*', '(', ')',
    ...
    '<', '.', ',', '?', '/'];
begin
  s := edtCreateAccountPassword.Text;
  ...
  for i in s do
  begin
    if (not i in numbers) and (not i in uppercase) and (not i in Lowercase)
      (not i in special_char) then
      error := 0;
  end;
