...
uses
  ...
  FMX.DateTimeCtrls, System.Dateutils;
...
type
  TForm1 = class(TForm)
    ...
    Layout1: TLayout;
    scrollSettings: TVertScrollBox;
    tContrSettings: TTabControl;
    general: TTabItem;
    user_interface: TTabItem;
    pnlSettingsGroups: TFlowLayout;
    rectSettingsGroup: TRectangle;
    ImageList4: TImageList;
    ToolBar1: TToolBar;
    account: TTabItem;
    flowUserInterface: TFlowLayout;
    rectScaleSettings: TRectangle;
    rectScaleSettingscollapse: TRectangle;
    ...
    lblScaleSettings: TLabel;
    radioNoScale: TRadioButton;
    radioAutoScale: TRadioButton;
    radioCustomScale: TRadioButton;
    ComboBox1: TComboBox;
    Label3: TLabel;
    ...
    procedure tContrSettingsChange(Sender: TObject);
...
begin
  ...
  rectSearch.Position.X := pnlDonationQuery.Width - rectSearch.Width -
    MultiView1.Width;
  tContrSettings.TabIndex := 0;
  tContrMain.TabIndex := 0;
  rectScaleSettings.Width := flowUserInterface.Width;
  Rectangle3.Width := flowUserInterface.Width;
  rectScaleSettingscollapse.Width := rectScaleSettings.Width;
  Rectangle4.Width := rectScaleSettingscollapse.Width;
...
begin
  ...
  scrollSettings.Width := ClientWidth - MultiView1.Width -
    pnlSettingsGroups.Width;
  rectSettingsGroup.Height := ClientHeight - pnlTop.Height -
    rectCopyright.Height;
...
procedure TForm1.LoadSettingsPanel;
var
  i: Integer;
begin
  rectScaleSettings.Width := flowUserInterface.Width + rectSettingsGroup.Width;
  Rectangle3.Width := flowUserInterface.Width + rectSettingsGroup.Width;
  rectScaleSettingscollapse.Width := rectScaleSettings.Width;
  Rectangle4.Width := rectScaleSettings.Width;
  for i := 1 to 8 do
  begin
    arrSettingsPanel[i] := TSettingsPanel.Create(Self, pnlSettingsGroups);
    arrSettingsPanel[i].setTabControl(tContrSettings);
    arrSettingsPanel[i].setTabIndex(i - 1);
  end;
  pnlSettingsGroups.Height := length(arrSettingsPanel) * 30;
  arrSettingsPanel[1].setClicked;
  arrSettingsPanel[1].SetLabel('General');
  arrSettingsPanel[2].SetLabel('User Interface');
  arrSettingsPanel[3].SetLabel('Account');
  // arrSettingsPanel[4].SetLabel('Inventory');
  // arrSettingsPanel[5].SetLabel('Report');
  // arrSettingsPanel[6].SetLabel('Backup');
  // arrSettingsPanel[7].SetLabel('Settings');
  // arrSettingsPanel[8].SetLabel('Help');
  arrSettingsPanel[1].GetTImage.Bitmap :=
    (ImageList4.Source.Items[0].MultiResBitmap.Bitmaps[1]);
  arrSettingsPanel[2].GetTImage.Bitmap :=
    (ImageList4.Source.Items[1].MultiResBitmap.Bitmaps[1]);
  arrSettingsPanel[3].GetTImage.Bitmap :=
    (ImageList4.Source.Items[2].MultiResBitmap.Bitmaps[1]);
  // arrSettingsPanel[4].GetTImage.Bitmap :=
  // (ImageList4.Source.Items[3].MultiResBitmap.Bitmaps[1]);
  // arrSettingsPanel[5].GetTImage.Bitmap :=
  // (ImageList4.Source.Items[4].MultiResBitmap.Bitmaps[1]);
  // arrSettingsPanel[6].GetTImage.Bitmap :=
  // (ImageList4.Source.Items[5].MultiResBitmap.Bitmaps[1]);
  // arrSettingsPanel[7].GetTImage.Bitmap :=
  // (ImageList4.Source.Items[6].MultiResBitmap.Bitmaps[1]);
  // arrSettingsPanel[8].GetTImage.Bitmap :=
  // (ImageList4.Source.Items[7].MultiResBitmap.Bitmaps[1]);
end;
...
procedure TForm1.tContrSettingsChange(Sender: TObject);
var
  i: Integer;
begin
  for i := 1 to 8 do
  begin
    if arrSettingsPanel[i].GetTabIndex <> tContrSettings.TabIndex then
    begin
      arrSettingsPanel[i].resetColor;
    end;
  end;
end;
