...
uses
  ...
  System.StrUtils, winApi.Windows, FMX.Platform, uSettingsPanel, ;
...
begin
  ...
  Rectangle1.Width := FlowLayout1.Width;
  Rectangle3.Width := FlowLayout1.Width;
  ...
  begin
    If (ClientWidth < MIN_WIDTH) AND (TabControl1.TabIndex = 1) Then
    Begin
      ClientWidth := MIN_WIDTH;
      Mouse_Event(MOUSEEVENTF_ABSOLUTE or MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
    End;

    If (ClientHeight < MIN_HEIGHT) AND (TabControl1.TabIndex = 1) Then
    Begin
...
begin
  ...
  begin
    ...
    begin;
      ...
      temp_head := StringReplace(DataModule1.ADOQuery1.Fields[i].FieldName, '_',
        ' ', [rfReplaceAll]);
      temp_head[1] := UpCase(temp_head[1]);
      temp_head[pos(' ', temp_head) + 1] :=
        UpCase(temp_head[pos(' ', temp_head) + 1]);
      StringGrid1.Columns[i].Header := temp_head;
    end;
...
var
  i: integer;
begin
  Rectangle1.Width := FlowLayout1.Width + rectSettingsGroup.Width;
  Rectangle3.Width := FlowLayout1.Width + rectSettingsGroup.Width;
  Rectangle2.Width := Rectangle1.Width;
  Rectangle4.Width := Rectangle1.Width;
  for i := 1 to 8 do
  begin
    arrSettingsPanel[i] := TSettingsPanel.Create(Self, pnlSettingsGroups);
    arrSettingsPanel[i].setTabControl(TabControl2);
...
begin
  Label1.Text := StringGrid1.Width.ToString;
end;
...
begin
  for i := 1 to 8 do
  begin
    if arrSettingsPanel[i].GetTabIndex <> TabControl2.TabIndex then
    begin
