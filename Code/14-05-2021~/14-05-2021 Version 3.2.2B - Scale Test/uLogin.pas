...
uses
  ...
  Data.Win.ADODB, System.NetEncoding, FMX.Memo;
...
function TForm2.GetPPI: integer;
var
  MetricsService: IFMXDeviceMetricsService;
  Metrics: TDeviceDisplayMetrics;
  PPI: integer;
begin
  if TPlatformServices.Current.SupportsPlatformService(IFMXDeviceMetricsService,
    MetricsService) then
  begin
    Metrics := MetricsService.GetDisplayMetrics;
    Result := Metrics.PixelsPerInch;
  end;
end;
...
begin
  ...
  Self.Height := Round(iniHeight*(ScreenSize.Height / 2160)*(2.25/SysScale));
end;

procedure TForm2.ScreenInfo;
var
  LDisplay: TDisplay;
  LPrimary: String;
  c: integer;
begin
  Memo1.Lines.Clear;
  for c := 0 to Screen.DisplayCount - 1 do
  begin
    LDisplay := Screen.Displays[c];
    LPrimary := '';
    if LDisplay.Primary then
    begin
      LPrimary := '(Primary)';
    end;
    Memo1.Lines.Add('Display: ' + Inttostr(LDisplay.Index) + ' ' +
      Inttostr(LDisplay.WorkArea.Size.cx) + ' x ' +
      Inttostr(LDisplay.WorkArea.Size.cy) + ' Scale: ' + (GetScreenScale)
      .ToString + ' Primary: ' + LPrimary + ' PPI: ' + Inttostr(GetPPI));
  end;
end;
...
begin
  Width := Round((TrackBar2.Value / 100) * iniWidth);
  Height := Round((TrackBar2.Value / 100) * iniHeight);
  Label23.Text := 'T: ' + floattostr(TrackBar2.Value) + '\n Width: ' +
    Inttostr(Width) + '\n Height: ' + Inttostr(Height);
end;
